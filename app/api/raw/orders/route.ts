...
type RawOrder = {
  ...
  status_code: number | null; seller_id: string | null; seller_assigned_at: string | null; current_total: number | null; net_total: number | null;
...
export async function GET(request: Request) {
  ...
  const rows = await env.DB.prepare(`SELECT id,source_order_id,pos_id,phone,customer_name,created_at,updated_at,status_code,seller_id,seller_assigned_at,current_total,net_total,first_confirmed_at,first_confirmed_by,delivered_at,history_limited,(raw_json IS NOT NULL) AS has_raw,order_source,fetched_at
