    ...
    <div className="space-y-5">
      ...
      {report && (
        <>
          ...
          {team !== 'all' && (report.coverage.unknownAuthorNotes ?? 0) > 0 && <p className="rounded-xl border border-[#f0dcb4] bg-[#fff8e8] px-4 py-3 text-sm text-[#8a5a00]">Trong kỳ có <strong>{vi.format(report.coverage.unknownAuthorNotes ?? 0)}</strong> ghi chú của {report.coverage.unknownAuthors} người viết không khớp nhân viên nào trong danh sách Pancake, nên không hiện khi lọc riêng Sale/CSKH. Chuyển bộ lọc sang "Tất cả" để thấy đủ; nếu người đó là nhân viên thật, hãy đặt bộ phận cho tài khoản trong Pancake.</p>}
