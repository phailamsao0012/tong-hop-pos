        ...
        extra={
          <>
            ...
            <Select value={callSort} items={{ perDay: 'Xếp: theo ngày', notes: 'Xếp: cuộc gọi', customers: 'Xếp: khách đã gọi', assigned: 'Xếp: data đang cầm', orders: 'Xếp: đơn chốt', net: 'Xếp: doanh thu', aov: 'Xếp: AOV', name: 'Xếp: tên' }} onValueChange={(v) => setCallSort(v as typeof callSort)}>
              <SelectTrigger className="min-w-36"><SelectValue /></SelectTrigger>
              <SelectContent>{Object.entries({ perDay: 'Theo ngày (cuộc/khách)', notes: 'Cuộc gọi', customers: 'Khách đã gọi', assigned: 'Data đang cầm', orders: 'Đơn chốt', net: 'Doanh thu', aov: 'AOV', name: 'Tên' }).map(([k, l]) => <SelectItem key={k} value={k}>{l}</SelectItem>)}</SelectContent>
            </Select>
      ...
      {report && (
        <>
          <div className="grid grid-cols-2 gap-2.5 sm:gap-4 xl:grid-cols-5">
            ...
            <KpiCard icon={Wallet} tone="orange" label="Đơn chốt (theo người bán)" value={vi.format(totals.orders)} note={`${money(totals.net)} · AOV ${totals.orders ? money(totals.net / totals.orders) : '—'}`} />
          ...
          <div id="call-history" className="scroll-mt-16">
            {selected && (
              <ChartCard icon={PhoneCall} title={`Lịch sử cuộc gọi · ${selected.name}`} subtitle={`${dmy(start)} – ${dmy(end)} · mỗi dòng một ghi chú; đơn chốt cùng ngày của khách đó gắn vào ghi chú đầu tiên trong ngày · tổng đơn chốt/doanh thu theo ngày tính theo người bán trên đơn`}
                        ...
                        <tbody>
                          {history.items.map((it) => (
                            <tr key={it.id} className="border-t align-top">
                              ...
                              <td className="whitespace-nowrap text-xs"><div className="max-w-[12rem] truncate font-medium" title={it.customer}>{it.customer || '—'}</div><div className="text-[#7d9184]">{it.phone ?? ''}{it.assignedTo ? ` · PC: ${it.assignedTo}` : ''}</div></td>
