        ...
        {cur && (
          <div className="grid grid-cols-12 gap-3" style={{ gridAutoRows: 'minmax(0, 1fr)', height: 'calc(100vh - 7.5rem)' }}>
            {[
              ...
              <div key={k.label} className="col-span-2 row-span-2 flex items-center gap-3 rounded-2xl border bg-white p-3"><span className={`grid size-10 shrink-0 place-items-center rounded-xl ${k.tone === 'blue' ? 'bg-[#e6f0fb] text-[#2a78d6]' : k.tone === 'green' ? 'bg-[#e4f5ea] text-[#17684b]' : k.tone === 'teal' ? 'bg-[#e1f5f0] text-[#0f8f74]' : k.tone === 'lime' ? 'bg-[#f1f8d6] text-[#5a7a12]' : k.tone === 'orange' ? 'bg-[#fdeee4] text-[#d85f2a]' : 'bg-[#ede9f9] text-[#5b48b8]'}`}><k.icon size={20} /></span><div className="min-w-0 flex-1"><div className="truncate text-xs text-[#6a8575]" title={k.label}>{k.label}</div><div className="whitespace-nowrap text-2xl font-semibold tracking-tight 2xl:text-3xl">{k.value}</div><div className="truncate text-xs text-[#7d9184]">{k.d !== null && k.d !== undefined ? <DeltaPill value={k.d} /> : null} {k.note ?? ''}</div></div></div>
            ))}
