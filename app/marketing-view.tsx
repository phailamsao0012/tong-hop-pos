...
type Summary = {
  ...
  /** Tiền các đơn hoàn / hủy trong phạm vi đang xem, và doanh thu còn lại sau khi trừ. */ refundNet: number; netAfterRefund: number;
};
...
type SortKey = 'name' | 'team' | 'created' | 'phones' | 'confirmed' | 'rate' | 'orders' | 'net' | 'refund' | 'afterRefund' | 'perPhone' | 'aov' | 'shipped' | 'delivered' | 'delivery' | 'returned';
...
const SORTS: Record<SortKey, string> = {
  name: 'Tên marketer', team: 'Team Marketing', created: 'Đơn tạo', phones: 'Số điện thoại', confirmed: 'Đơn xác nhận', rate: 'Tỷ lệ xác nhận', orders: 'Đơn theo mốc', net: 'Doanh thu', refund: 'Tiền hoàn / hủy', afterRefund: 'Doanh thu sau hoàn hủy', perPhone: 'Doanh thu / SĐT',
...
export function MarketingView({ onManageTeams }: { onManageTeams?: () => void }) {
  ...
  const rows = useMemo(() => sort.apply(data?.byMarketer ?? [], (r, k) => {
    switch (k) {
      ...
      case 'refund': return r.refundNet;
      case 'afterRefund': return r.netAfterRefund;
  ...
  const exportExcel = async () => {
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ['Marketer', 'Team Marketing', 'Đơn tạo', 'SĐT trên đơn', 'Đơn xác nhận', 'Tỷ lệ xác nhận %', `Đơn: ${STAGES[stage]}`, 'Doanh thu theo mốc', 'Tiền hoàn / hủy', 'Doanh thu sau hoàn hủy', 'Doanh thu / SĐT', 'AOV', 'Đã cho ĐVVC', 'Đã nhận', 'Tỷ lệ giao TC %', 'Hoàn', 'Hủy'],
      ...rows.map((r) => [r.marketerName, r.marketingTeamName, r.createdOrders, r.createdPhones, r.confirmedOrders, r.confirmationRate ?? '', r.orders, r.net, r.refundNet, r.netAfterRefund, Math.round(r.revenuePerPhone ?? 0), Math.round(r.averageOrder ?? 0), r.shippedOrders, r.deliveredOrders, r.deliveryRate ?? '', r.returnedOrders, r.cancelledOrders]),
    ]), 'Hiệu suất marketer');
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([
      ['Team Marketing', 'Marketer có dữ liệu', 'Đơn tạo', 'SĐT duy nhất trong team', 'Đơn xác nhận', 'Tỷ lệ xác nhận %', `Đơn: ${STAGES[stage]}`, 'Doanh thu theo mốc', 'Tiền hoàn / hủy', 'Doanh thu sau hoàn hủy', 'Doanh thu / SĐT', 'AOV', 'Đã cho ĐVVC', 'Đã nhận', 'Tỷ lệ giao TC %', 'Hoàn', 'Hủy'],
      ...teamRows.map((r) => [r.marketingTeamName, r.marketerCount, r.createdOrders, r.createdPhones, r.confirmedOrders, r.confirmationRate ?? '', r.orders, r.net, r.refundNet, r.netAfterRefund, Math.round(r.revenuePerPhone ?? 0), Math.round(r.averageOrder ?? 0), r.shippedOrders, r.deliveredOrders, r.deliveryRate ?? '', r.returnedOrders, r.cancelledOrders]),
      ...
      {loading && !data ? <SkeletonKpis count={6} /> : s ? <>
        ...
        <ChartCard icon={BarChart3} title={rankingMode === 'team' ? 'Xếp hạng team Marketing' : 'Xếp hạng marketer'} subtitle={`Chỉ số theo mốc: ${STAGES[stage]} · phễu theo đơn tạo trong kỳ`}
          ...
          {rankingMode === 'team' ? (teamRows.length ? <>
            ...
            <TableWrap><table className="tbl"><thead><tr><th>#</th><SortTh k="name" label="Team Marketing" sort={sort} align="left" /><th className="n">Marketer</th><SortTh k="created" label="Đơn tạo" sort={sort} /><SortTh k="phones" label="SĐT" sort={sort} /><SortTh k="confirmed" label="Đơn XN" sort={sort} /><SortTh k="rate" label="Tỷ lệ XN" sort={sort} /><SortTh k="orders" label="Đơn theo mốc" sort={sort} /><SortTh k="net" label="Doanh thu" sort={sort} /><SortTh k="refund" label="Hoàn / hủy (tiền)" sort={sort} /><SortTh k="afterRefund" label="Sau hoàn hủy" sort={sort} /><SortTh k="perPhone" label="DT / SĐT" sort={sort} /><SortTh k="aov" label="AOV" sort={sort} /><SortTh k="shipped" label="Đã cho ĐVVC" sort={sort} /><SortTh k="delivered" label="Đã nhận" sort={sort} /><SortTh k="delivery" label="% giao TC" sort={sort} /><SortTh k="returned" label="Hoàn / hủy" sort={sort} /></tr></thead>
              <tbody>{teamRows.map((r, i) => <tr key={r.marketingTeamId}><td className="num text-ink-3">{i + 1}</td><td><button type="button" className="font-semibold text-primary underline-offset-2 hover:underline" onClick={() => { setMarketingTeamId(r.marketingTeamId); setMarketerId('__all'); setRankingMode('marketer'); }}>{r.marketingTeamName}</button></td><td className="n">{vi.format(r.marketerCount)}</td><td className="n">{vi.format(r.createdOrders)}</td><td className="n">{vi.format(r.createdPhones)}</td><td className="n">{vi.format(r.confirmedOrders)}</td><td className="n">{pct(r.confirmationRate)}</td><td className="n">{vi.format(r.orders)}</td><td className="n">{money(r.net)}</td><td className="n text-bad">{r.refundNet ? `−${money(r.refundNet)}` : money(0)}</td><td className="n font-semibold">{money(r.netAfterRefund)}</td><td className="n">{money(r.revenuePerPhone)}</td><td className="n">{money(r.averageOrder)}</td><td className="n">{vi.format(r.shippedOrders)}</td><td className="n">{vi.format(r.deliveredOrders)}</td><td className="n">{pct(r.deliveryRate)}</td><td className="n">{vi.format(r.returnedOrders)} / {vi.format(r.cancelledOrders)}</td></tr>)}</tbody>
            ...
            <tbody>{rows.map((r, i) => <tr key={r.marketerId}><td className="num text-ink-3">{i + 1}</td><td className="font-medium">{r.marketerName}</td><td className="whitespace-nowrap text-xs text-ink-2">{r.marketingTeamName}</td><td className="n">{vi.format(r.createdOrders)}</td><td className="n">{vi.format(r.createdPhones)}</td><td className="n">{vi.format(r.confirmedOrders)}</td><td className="n">{pct(r.confirmationRate)}</td><td className="n">{vi.format(r.orders)}</td><td className="n">{money(r.net)}</td><td className="n text-bad">{r.refundNet ? `−${money(r.refundNet)}` : money(0)}</td><td className="n font-semibold">{money(r.netAfterRefund)}</td><td className="n">{money(r.revenuePerPhone)}</td><td className="n">{money(r.averageOrder)}</td><td className="n">{vi.format(r.shippedOrders)}</td><td className="n">{vi.format(r.deliveredOrders)}</td><td className="n">{pct(r.deliveryRate)}</td><td className="n">{vi.format(r.returnedOrders)} / {vi.format(r.cancelledOrders)}</td></tr>)}</tbody>
