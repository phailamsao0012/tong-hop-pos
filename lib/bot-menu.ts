...
import { handleCommand, HELP } from '@/lib/bot';
...
export const MAIN_MENU: InlineKeyboard = {
  inline_keyboard: [
    ...
    [{ text: '🔍 Tìm khách', callback_data: 'kh:hint' }, { text: '🔄 Đồng bộ', callback_data: 'sync' }],
...
/** Xử lý dữ liệu nút bấm → nội dung + bàn phím mới. */
export async function handleCallback(data: string, userName: string): Promise<{ text: string; keyboard: InlineKeyboard }> {
  const [kind, a, b, c] = data.split(':');
  if (kind === 'menu') return startScreen(userName);
  if (kind === 'help') return { text: HELP, keyboard: withBack([]) };
  if (kind === 'sync') return { text: (await handleCommand('/dongbo'))[0], keyboard: withBack([[{ text: '🔄 Làm mới', callback_data: 'sync' }]]) };
  ...
  if (kind === 'bc') return { text: (await handleCommand(`/baocao ${a}`))[0], keyboard: withBack([periodRow('bc', a)]) };
  if (kind === 'pos') return { text: (await handleCommand(`/pos ${a}`))[0], keyboard: withBack([periodRow('pos', a)]) };
  if (kind === 'top') return { text: (await handleCommand(`/top ${a}`))[0], keyboard: withBack([periodRow('top', a), [{ text: 'Bộ phận CSKH', callback_data: `topd:${a}:cskh` }, { text: 'Bộ phận SALE', callback_data: `topd:${a}:sale` }]]) };
  if (kind === 'topd') return { text: (await handleCommand(`/top ${a} ${b}`))[0], keyboard: withBack([periodRow('top', a)]) };
  if (kind === 'sp') return { text: (await handleCommand(`/sanpham ${a}`))[0], keyboard: withBack([periodRow('sp', a)]) };
  if (kind === 'ml') return { text: (await handleCommand(`/mualai ${a}`))[0], keyboard: withBack([periodRow('ml', a)]) };
  if (kind === 'cn') {
    const period = a === 'ca' ? '' : a;
    return { text: (await handleCommand(`/chotnong ${period}`))[0], keyboard: withBack([[{ text: a === 'ca' ? '• Ca hôm nay' : 'Ca hôm nay', callback_data: 'cn:ca' }, ...periodRow('cn', a).slice(0, 3)]]) };
  }
  ...
  if (kind === 'nv' && a === 'id') {
    ...
    const text = name ? (await handleCommand(`/nhanvien ${name} ${period}`))[0] : 'Không tìm thấy nhân viên.';
