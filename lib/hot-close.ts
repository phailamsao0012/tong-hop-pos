...
export async function hotCloseByEmployee(
  ...
  /** Ca cá nhân: khung giờ riêng (UTC ISO) của từng nhân viên; null = dùng nguyên khung chung. */
  windowOf?: (employeeId: string) => [string, string] | null,
) {
  ...
  const inWindow = (id: string, at: string) => { const w = windowOf?.(id); return !w || (at >= w[0] && at < w[1]); };
  ...
  for (const a of assigned.results as A[]) {
    if (!inWindow(a.seller_id, a.seller_assigned_at)) continue;
    const phone = normalizePhone(a.phone);
    if (phone) row(a.seller_id).receivedPhones.add(`${a.pos_id}:${phone}`);
  }
  for (const c of confirmed.results as C[]) {
    const phone = normalizePhone(c.phone);
    if (!phone || !inWindow(c.closer_id, c.first_confirmed_at)) continue;
